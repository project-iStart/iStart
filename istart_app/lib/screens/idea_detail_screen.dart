...
class _FounderCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final founderId = (founder['_id'] ?? founder['id'] ?? '') as String;
      ...
      onTap: founderId.isEmpty
          ...
          : () {
              Navigator.of(context).push(
                MaterialPageRoute(
                  builder: (_) =>
                      PublicProfileScreen(userId: founderId, userName: name),
                ),
              );
            },
      child: Container(
      ...
      onTap: memberId.isEmpty
          ...
          : () {
              Navigator.of(context).push(
                MaterialPageRoute(
                  builder: (_) =>
                      PublicProfileScreen(userId: memberId, userName: name),
                ),
              );
            },
      child: Container(
...
class _FundingRequestButton extends StatefulWidget {
  const _FundingRequestButton({required this.idea, required this.accent});

  final StartupIdea idea;
  final Color accent;

  @override
  State<_FundingRequestButton> createState() => _FundingRequestButtonState();
}

class _FundingRequestButtonState extends State<_FundingRequestButton>
    with SingleTickerProviderStateMixin {
  late AnimationController _ctrl;
  late Animation<double> _scale;
  ...
  Future<void> _onTap() async {
    ...
    showDialog(
      context: context,
      builder: (context) => SendFundingRequestDialog(
        ideaId: widget.idea.id,
        ideaTitle: widget.idea.title,
      ),
    );
...
// ─── Manage Investment Requests Button ────────────────────────────────────────

class _ManageInvestmentRequestsButton extends StatefulWidget {
  const _ManageInvestmentRequestsButton({
    required this.idea,
  ...
  @override
  State<_ManageInvestmentRequestsButton> createState() =>
      _ManageInvestmentRequestsButtonState();
}

class _ManageInvestmentRequestsButtonState
    extends State<_ManageInvestmentRequestsButton>
    with SingleTickerProviderStateMixin {
  late AnimationController _ctrl;
  late Animation<double> _scale;
  ...
  Future<void> _onTap() async {
    ...
    Navigator.of(context).push(
      MaterialPageRoute(
        builder: (_) => InvestmentRequestManagementScreen(idea: widget.idea),
      ),
    );
  }
